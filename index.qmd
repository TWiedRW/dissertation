...
#| label: fig-wsj
#| layout-ncol: 2
#| fig-scap: "3D graphic from the Wall Street Journal"
#| out-width: 90%
#| fig-cap: "3D-printed volume charts showing the sales of 3D printers under $5,000 [@thingiversecomChartSales3D]."
#| fig-subcap: 
#| - "Original WSJ 3D print (2014)"
#| - "Replicated WSJ 3D print (2025)"


knitr::include_graphics('images/wsj-print.jpg')
knitr::include_graphics('images/wsj-myprint.jpg')
```
...
![2D bar face](images/make-bar-3d-1.png){width=90% fig-scap}

![Opposite bar face](images/make-bar-3d-2.png){width=90%}

![Connecting bar faces](images/make-bar-3d-3.png){width=90%}

![Solid 3D bar](images/make-bar-3d-4.png){width=90%}

Construction of depth cues for a 2D bar chart where the y-axis corresponds to the front face of the bar.
:::
...
Field-of-view parameters for volcano dataset [@volcano].
:::
...
Two examples of 3D bar charts found in publication [@cdi_hathitrust_hathifiles_uva_x030492864].
:::
...
Heat maps are a type of chart that can be used with two nominal or ordinal variables, and a continuous response variable. In two-dimensional formats, gradient fills are often used to denote the magnitude of the responses, utilizing a gradient scale. When represented in three dimensions, height is the primary mapping of the response of the heatmap. Due to the height aesthetic, 3D heat maps are sometimes referred to as "height maps", particularly when spatial coordinates are involved. Since the color and/or height of the response variable can only be mapped once to each combination of x and y coordinates, heat maps are generally used to summarize responses, such as averages or single-point observations. This makes heat maps especially useful for visualizing patterns and trends across large datasets in a compact and interpretable format.

```{r}
#| label: fig-heatmap2d
#| fig-cap: "2D and 3D heat maps from @national1977condition"
...
Height-scaling effects in 3D heat maps for the volcano dataset [@volcano].
:::
